-- Migración — guardar_encuesta_completa(): captura real de iniciada_en
--
-- Hoy inserta iniciada_en y completada_en con el mismo now(), así que la
-- duración real de una encuesta nunca se capturó. Se agrega un parámetro
-- opcional al final (p_iniciada_en) con default null — si el cliente no lo
-- manda, se comporta exactamente igual que hoy (compatible con la versión de
-- la app ya instalada en los celulares, que llama a esta RPC con 7
-- argumentos). Ver PLAN-tiempo-encuestas-y-mensajes.md, sección 1.
--
-- Es un parámetro agregado al final con default: CREATE OR REPLACE alcanza,
-- no cambia la identidad de la función ni requiere DROP.
-- Rollback: 20260906022901_guardar_encuesta_completa_iniciada_en_rollback.sql
