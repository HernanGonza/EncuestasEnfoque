-- get_respuestas_crudas(): sumar zona_id/zona_nombre a cada fila.
--
-- Sección 8 del plan (módulo de reportes): varios de los 10 reportes
-- automáticos necesitan agrupar por zona (comparativo de candidatos por
-- zona, completo por pregunta y zona, no-respuesta por zona, distribución
-- geográfica), y esta RPC —ya usada hoy por "Datos crudos" en
-- Reportes.jsx— trae todo lo demás (encuestador, equipo, fecha, lat/lng,
-- respuestas resueltas) pero no la zona. Se agrega sin tocar nada del
-- resto: mismo shape de columnas/filas, misma firma, mismos filtros.
--
-- CREATE OR REPLACE porque no cambia la firma ni el tipo de retorno —
-- compatible con el código ya desplegado (los callers existentes ignoran
-- las claves nuevas del objeto).
--
-- Ver PLAN-tiempo-encuestas-y-mensajes.md, sección 8.
-- Rollback: 20260906023300_get_respuestas_crudas_zona_rollback.sql
