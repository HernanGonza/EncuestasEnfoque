-- Migración — get_stats_encuestadores_por_encuesta(): promedio_segundos por encuestador
--
-- Agrega una columna nueva al RETURNS TABLE (promedio_segundos numeric).
-- Cambiar las columnas de salida de una función requiere DROP + CREATE
-- (CREATE OR REPLACE no permite cambiar el tipo de retorno).
-- Ver PLAN-tiempo-encuestas-y-mensajes.md, sección 1.
--
-- Mismo criterio que la migración anterior: se excluyen sesiones con
-- completada_en = iniciada_en (dato viejo sin timing real).
-- Rollback: 20260906022903_stats_encuestadores_tiempo_rollback.sql

DROP FUNCTION IF EXISTS public.get_stats_encuestadores_por_encuesta(uuid, uuid);

CREATE FUNCTION public.get_stats_encuestadores_por_encuesta(p_encuesta_id uuid, p_equipo_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(encuestador_id uuid, completadas integer, no_respuesta integer, total integer, cuota integer, promedio_segundos numeric)
  ...
  sesiones AS (
    SELECT
      ae.encuestador_id,
      sr.id AS sesion_id,
      CASE WHEN sr.completada_en > sr.iniciada_en
        THEN EXTRACT(EPOCH FROM (sr.completada_en - sr.iniciada_en))
        ELSE NULL
  ...
  SELECT
    ...
    AVG(s.duracion_segundos)                                    AS promedio_segundos
